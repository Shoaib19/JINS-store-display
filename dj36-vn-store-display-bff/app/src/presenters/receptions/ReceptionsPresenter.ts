...
import { calcWaitingStatus,filterWaitingReceptions } from "~/src/utils/calculateWaitingTime";
  ...
  implements IReceptionsPresenter
{
  ...
  > => {
    try {
      ...
      const filteredReceptions = filterWaitingReceptions(receptionsGetResponseData.receptionInfos);
      ...
      logger.info(
        `Filtered receptions. Total: ${filteredReceptions}, Filtered: ${otherReceptionInfos.length}`,
      );
