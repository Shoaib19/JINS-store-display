...
import { SubEventCode } from "~/src/components/eventCode";
...
import { COUNTRY_CODE_ALPHA2, ReplacementPart } from "~/src/components/const";
import { ApiResponse } from "openapi-typescript-fetch";
import { components } from "~/src/interfaces/root";

// 保証交換
export type WarrantyExchange = components["schemas"]["warrantyExchange"];
...
// （カート）Suiteクラスで保持する形式
type ItemGroupExtendsLineitems = Omit<ItemGroup, "Lineitems">;
...
export class Suite {
   ...
   * @returns
   */
  public isCerritoRegistered() {
    // セリートのカート明細があること
    if (this.items["cerrito"] != undefined) {
      return true;
    }
    return false;
  }
   ...
   * @returns
   */
  public hasRequiredPrescriptionImage() {
  ...
  public async getLensItemCode(
    ...
    logger.info(`getLensUniqueAttributesRequest: ${JSON.stringify(lensUniqueAttributesGetRequest)}`);
    ...
    const lensUniqueAttributesGetResponse = apiResponse.data;
  ...
  private async isOutOfStockItemInStore(
    ...
    const _requestHeader = makeDpfmRequestHeader(dpfmRequestInfo);
    const _getInventoryRequest: InventoriesGetRequest = {
      locationCodeList: ["83005"], //NOTE: static value カート情報取得APIで取得予定
      itemCodeList: itemCodes,
      salesStatusIdList: [1], //NOTE: saleable
      itemStatusIdList: [1], //NOTE: enough quality
    };
    return false;

    // logger.info(`getInventoriesRequest: ${JSON.stringify(getInventoryRequest)}`);
    // const apiResponse: ApiResponse<InventoriesGetResponse> = await sendApiRequest(
    //   getInventoriesServer,
    //   getInventoryRequest,
    //   requestHeader
    // );
    // logger.info(`getInventoriesResponse: ${JSON.stringify(apiResponse)}`);

    // const inventoriesGetResponse = apiResponse.data;
  ...
  private updateItemAmount(
    ...
    // 価格が変更されたかを判定
    const isAmountChanged = (item: Lineitem, itemAmount: ItemAmount) => {
      return (
    ...
    const item = this.getLineItem(itemType);
      if (item != undefined) {
        if (isAmountChanged(item, itemAmount)) {
