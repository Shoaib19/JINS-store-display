 ...
 * カート登録・更新（セリート）モデル
 */
export class SalesRegisterCerritoModel extends SalesRegisterModelTemplate {
  /**
   * バリデーションチェック
  ...
  protected override mergeRequest(
    ...
    suite.setEventCode(EventCode.CERRITO.CODE);
    if (itemGroupRequestInfo.cerritoCode !== undefined) {
      let cerritoItem = suite.getLineItem("cerrito");
      // セリートコードが指定されているときはセリートを登録する。
      if (cerritoItem == undefined) {
        cerritoItem = {
          operationCode: "01", // 登録
          itemCode: itemGroupRequestInfo.cerritoCode??undefined,
        };
        suite.setSubEventCode(SubEventCode.ADD);
      } else {
        cerritoItem.itemCode = itemGroupRequestInfo.cerritoCode??undefined;
        cerritoItem.operationCode ??= "02"; // 更新
        suite.setSubEventCode(SubEventCode.CHANGE);
      }
      cerritoItem.preparation1 =
        itemGroupRequestInfo.cerritoCode == null ? "cerrito None." : undefined; // セリートコードがnullの場合固定文言を設定
  ...
  protected override async verify(
    ...
    const verifyFn: ((
      ...
      //  6.1 フレーム・メガネケース・セリートの全国在庫照会（12-4スコープでも対象外）
      this.checkFrameStockInStore, // 6.2 フレーム・メガネケース・セリートの店舗在庫照会(フレーム)
      this.checkCaseStockInStore, // 6.2 フレーム・メガネケース・セリートの店舗在庫照会(ケース)
      this.checkCerritoStockInStore // 6.2 フレーム・メガネケース・セリートの店舗在庫照会(セリート)
    ];
