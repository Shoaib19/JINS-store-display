...
import { WARRANTY_PERIOD_MONTH } from "~/src/components/const";
...
export const roundUpToNextMinute = (date: Date): Date => {
  ...
  const roundedDate =
    date.getSeconds() > 0 ? dateFns.addMinutes(date, 1) : new Date(date);
  // 秒・ミリ秒を0に設定
...
export const adjustTimeZone = (utcDate: Date, timeZone?: string) => {
  if (timeZone == undefined) {
    ...
    const localDate = new TZDate(Date.UTC(
 ...
 */
export const getTimeOfToday = (timeLocal: string | null | undefined, timeZone: string) => {
 ...
 */
export const getStartOfDate = (date: Date| null | undefined, timeZone: string): Date | null => {
  if (!date) return null;
 ...
 * @param timeZone タイムゾーン
 * @returns タイムゾーンにおける保証終了日の0:00(UTC時刻)
 */
export const getWarrantyExpirationDate = (
  ...
  zonedDate.setUTCHours(0, 0, 0, 0);
  // 保証の有効期間(月)から有効期限を計算する(閏年は自動的に考慮される)
  const warrantyExpirationDate = dateFns.addMonths(zonedDate, WARRANTY_PERIOD_MONTH);
