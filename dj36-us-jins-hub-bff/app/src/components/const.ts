...
export const DEFAULT_STORE_TIME_ZONE = "America/Los_Angeles";
// 国のタイムゾーン(IANA or UTC offset)
export const COUNTRY_TIME_ZONE = "-06:00";  // CST
// 国名コード(ISO 3166-1 alpha-2)
export const COUNTRY_CODE_ALPHA2 = "US";
// 通貨コード(ISO 4217)
...
/**
 * 受付ステータス名(reception status name)
 */
export const ReceptionStatusName = {
    /** 度数測定(100) */
    ...
    COMPLETED: "completed",
    /** 注文完了(801) **/
    ORDER_COMPLETE: "reception completed",
    /** 受付キャンセル(900) **/
    CANCEL: "Reception Cancel",
    /** オーダーキャンセル(901) **/
...
/**
 * 受け渡し方法での保証開始日を遅らせる日数
 */
export const WarrantyStartDelayDays = {
  // 受け渡し方法が配送
  DELIVERY_METHOD_SHIPPING: 3,
};

/**
 * 保証の有効期間(月)
 */
export const WARRANTY_PERIOD_MONTH = 12;

/**
 * お渡し予定日を遅らせる日数
 */
export const HandsOverDelayDays = {
  // レンズが特注品の場合
  // プレオープン直後は特注レンズのお渡し日を+21日にする(DJ36_STORE-1044)
  SPECIAL_ORDER_LENS: 21,
};

/**
 * デジタル基盤で在庫管理するかを設定
 * (true: 在庫管理する、 false: 在庫管理しない)
 */
export const USE_INVENTORY = true;
...
export const SMSConfig = {
  // 受付完了のSMS通知
  receptionCompleteMessageEnabled: true,
  // 受付完了時のアプリダウンロードののSMS通知
  appDownloadMessageEnabled: true,

  // Eye exam/Prescription
  // 呼出のSMS通知
  eyeExamCallingMessageEnabled: true,
  // もうすぐ呼出のSMS通知
  eyeExamCallingSoonMessageEnabled: true,

  // No eye exam/Prescription・Adjustment・General Help
  // 呼出のSMS通知
  adjustmentCallingMessageEnabled: true,
  // もうすぐ呼出のSMS通知
  adjustmentCallingSoonMessageEnabled: false,

  // お渡し準備完了ののSMS通知
  readyForPickupMessageEnabled: true,
}
