...
type ApiMethod = {
  ...
  method: string;
  service: string;
  call: (params: any, fetchOptions?: RequestInit) => Promise<any>;
};
 ...
 * @returns ApiMethod
 */
export const createApiMethod = (path: string, method: string, service: string): ApiMethod => {
  const call = async (params: any, fetchOptions?: RequestInit) => {
    return fetcher
      .path(path)
      .method(method)
...
const getRetrySettings = () => {
  const retries = parseInt(process.env.API_RETRIES || '1', 10);
 ...
 */
const retryApiRequest = async (call: (params: any, fetchOptions?: RequestInit) => Promise<any>, params: any, fetchOptions?: RequestInit) => {
 ...
 */
export const sendApiRequest = async (apiMethod: ApiMethod, params: any, headers?: Headers, url?: string) => {
